...
static big::Keyboard g_keyboard;
namespace big {
	void Keyboard::OnKeyboardMessage(DWORD key, WORD repeats, uint8_t scan_code, BOOL is_extended, BOOL is_with_alt, BOOL was_down_before, BOOL is_up_now) {
		/*Numpad shift fix*/
		if (key == 38) key = 104;
		if (key == 39) key = 102;
		if (key == 37) key = 100;
