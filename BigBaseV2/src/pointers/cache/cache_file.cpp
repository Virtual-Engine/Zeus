...
namespace big
{
	cache_file::cache_file(std::filesystem::path cache_file) :
	...
	cache_file::cache_file(std::filesystem::path cache_file, uint32_t cache_version) :
		m_cache_file(cache_file),
		m_data(nullptr),
	...
	bool cache_file::load()
	{
		...
		auto file = std::ifstream(get_path(), std::ios::binary);
	...
	bool cache_file::write() const
	{
		...
		auto file = std::ofstream(get_path(), std::ios::binary);
